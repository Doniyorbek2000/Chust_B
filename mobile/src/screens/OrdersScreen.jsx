...
export default function OrdersScreen({ navigation }) {
  ...
  return (
    <FlatList
      ...
      ListEmptyComponent={
        <Empty icon="📦" title="Buyurtmalar yo'q" text="Siz hali buyurtma bermagansiz"
          action={<Button title="Xarid qilish" style={{ marginTop: 10, paddingHorizontal: 34 }}
          ...
          <TouchableOpacity style={s.card} activeOpacity={0.8}
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <View style={[s.status, { backgroundColor: st.bg }]}>
                <Text style={{ color: st.color, fontSize: 12, fontWeight: '700' }}>{st.icon} {st.label}</Text>
              </View>
            </View>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 }}>
              <Text style={{ color: colors.ink2 }}>{item.items.length} ta mahsulot</Text>
